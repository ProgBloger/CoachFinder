...
export default({
    ...
    methods: {
        ...
        validateForm() {
            ...
            const isNullOrNegative = val => val === null || val < 0;
            const isEmptyArray = val => val.length === 0;

            const validations = [
                { field: 'firstName', validate: isEmpty },
                { field: 'lastName', validate: isEmpty },
                { field: 'description', validate: isEmpty },
                { field: 'rate', validate: isNullOrNegative },
                { field: 'areas', validate: isEmptyArray },
            ];

            this.formIsValid = true;

            validations.forEach(this.validateField);
        },

        validateField(validation) {
            const { field, validate } = validation;
            if(validate(this[field].val)) {
